...
- ~~**No convertir el "+ Añadir línea"** en buscador/combobox~~ — **SUPERADO (18 jul 2026, decisión del cliente):** con el catálogo real de clientes el desplegable se hizo pesado. El paso de elegir cliente pasa a ser un **buscador predictivo sin librerías** (filtrado insensible a acentos, teclado ↓/↑/Enter, ARIA listbox) con un grupo **"Recientes"** por usuario (derivado de sus horas de las últimas 6 semanas, que ya se cargaban — `clientesRecientes` en `cargarMiSemana`). El resto del flujo (checkboxes de proyectos, "Añadir N líneas") no cambia. La regla original sigue vigente en espíritu: nada de librerías de combobox.
